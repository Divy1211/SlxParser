...
def set_pixel_data_repeats():
    num_bytes = get(ret(SldPlayerColorMaskLayer.header), ret(SldShortLayerHeader.num_bytes))
    num_draw_commands = get_len(ret(SldPlayerColorMaskLayer.draw_commands))

    total_num_bytes = num_bytes + ((4 - num_bytes) % 4)

    return [
        set_repeat(ret(SldPlayerColorMaskLayer.pixels)).by((num_bytes - 2*num_draw_commands - 8)//8),
        set_repeat(ret(SldPlayerColorMaskLayer._null)).by(total_num_bytes - num_bytes),
    ]


class SldPlayerColorMaskLayer(BaseStruct):
    ...
    pixels: list[Bc4Block] =              Retriever(Bc4Block,                default_factory = Bc4Block, repeat = 0)

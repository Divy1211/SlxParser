...
from sld.sections.sld_frame.unknown_layer import SldUnknownLayer
...
def set_layer_repeats():
    ...
    return [
        ...
        if_(*frame_type).eq(get(*frame_type) & (~outline_bit & 0xFF)).then(set_repeat(ret(SldFrame.unknown_layer)).to(-1)),
...
class SldFrame(BaseStruct):
    ...
    unknown_layer: SldUnknownLayer | None =                   Retriever(SldUnknownLayer, default_factory = SldUnknownLayer)

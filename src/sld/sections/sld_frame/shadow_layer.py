...
def set_pixel_data_repeats():
    num_bytes = get(ret(SldShadowLayer.header), ret(SldLayerHeader.num_bytes))
    num_draw_commands = get_len(ret(SldShadowLayer.draw_commands))

    total_num_bytes = num_bytes + ((4 - num_bytes) % 4)

    return [
        set_repeat(ret(SldShadowLayer.pixels)).by((num_bytes - 2*num_draw_commands - 16)//8),
        set_repeat(ret(SldShadowLayer._null)).by(total_num_bytes - num_bytes),
    ]


class SldShadowLayer(BaseStruct):

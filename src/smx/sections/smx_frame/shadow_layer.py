...
class SmxShadowLayer(BaseStruct):
    ...
    # @formatter:off
    header: SmxLayerHeader    = Retriever(SmxLayerHeader, default_factory = SmxLayerHeader, on_read = lambda: [set_repeat(ret(SmxShadowLayer.row_edges)).from_(ret(SmxShadowLayer.header), ret(SmxLayerHeader.height))])
